...
req_del() {
    cat /etc/samvpn/xray/user.txt >/tmp/cad.${message_from_id[$id]}
    alluser=$(cat /etc/samvpn/xray/user.txt | awk '{print $1}' | sort | uniq)
    ShellBot.sendMessage --chat_id ${callback_query_from_id[$id]} \
        --text "━━━━━━━━━━━━━━━━━━━━━\n<b>🔸 Del ACCOUNT 🔸 </b>\n━━━━━━━━━━━━━━━━━━━━━\n\n$alluser\n\n━━━━━━━━━━━━━━━━━━━━━\n" \
        --parse_mode html
    ShellBot.sendMessage --chat_id ${callback_query_from_id[$id]} \
...
req_limit() {
    limituser=$(sed -n '2 p' /root/multi/bot.conf | cut -d' ' -f2)
    total=$(wc -l /etc/samvpn/xray/user.txt | cut -d' ' -f1)
    coupon=$(grep 'start [^_]*' $file_user | grep -o '[^_]*' | cut -d' ' -f2 | sed -n '3p')
