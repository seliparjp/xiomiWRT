...
function MediaUnlockTest_Netflix() {
    ...
    if [[ "$result1" == "404" ]]; then
        ...
        echo -n -e "\r Netflix:\t\t\t\t${Font_Green}Yes (${region})${Font_Suffix}\n"
        return
...
function MediaUnlockTest_DisneyPlus() {
    ...
    if [[ "$region" == "JP" ]]; then
        echo -n -e "\r Disney+:\t\t\t\t${Font_Green}Yes (JP)${Font_Suffix}\n"
        return
        ...
        echo -n -e "\r Disney+:\t\t\t\t${Font_Green}Yes ($region)${Font_Suffix}\n"
        return
    elif [ -z "$region" ]; then
        echo -n -e "\r Disney+:\t\t\t\t${Font_Red}No${Font_Suffix}\n"
        return
...
function MediaUnlockTest_Dazn() {
    ...
    if [[ "$isAllowed" == "true" ]]; then
        ...
        echo -n -e "\r Dazn:\t\t\t\t\t${Font_Green}Yes (${CountryCode})${Font_Suffix}\n"
        return
...
function MediaUnlockTest_HBOMax() {
    ...
    if [ -n "$isUnavailable" ]; then
        ...
        echo -n -e "\r HBO Max:\t\t\t\t${Font_Green}Yes ($region)${Font_Suffix}\n"
        return
...
function MediaUnlockTest_Viu.com() {
    ...
    if [ -n "$result" ]; then
        if [[ "$result" == "no-service" ]]; then
            ...
            result=$(echo $result | tr [:lower:] [:upper:])
            echo -n -e "\r Viu.com:\t\t\t\t${Font_Green}Yes (${result})${Font_Suffix}\n"
            return
        fi
...
function MediaUnlockTest_Niconico() {
    ...
    echo $tmpresult | grep '????' >/dev/null 2>&1
    if [[ "$?" -eq 0 ]]; then
...
function MediaUnlockTest_YouTube_Premium() {
    ...
    if [ -n "$result" ]; then
        echo -n -e "\r YouTube Premium:\t\t\t${Font_Red}No${Font_Suffix} ${Font_Green} ($region)${Font_Suffix} \n"
        return

    fi
    local result=$(echo $tmpresult | grep 'manageSubscriptionButton')
    if [ -n "$result" ]; then
        echo -n -e "\r YouTube Premium:\t\t\t${Font_Green}Yes ($region)${Font_Suffix}\n"
        return
    else
...
function MediaUnlockTest_PrimeVideo_Region() {
    ...
    if [ -n "$result" ]; then
        echo -n -e "\r Amazon Prime Video:\t\t\t${Font_Green}Yes ($result)${Font_Suffix}\n"
        return
    else
...
function MediaUnlockTest_HotStar() {
    ...
    if [ "$result" = "000" ]; then
        ...
        if [ -n "$region" ] && [ "$region" = "$site_region" ]; then
            echo -n -e "\r HotStar:\t\t\t\t${Font_Green}Yes ($region)${Font_Suffix}\n"
            return
        else
...
function MediaUnlockTest_HBOGO_ASIA() {
    ...
    if [ -z "$result" ]; then
        ...
        echo -n -e "\r HBO GO Asia:\t\t\t\t${Font_Green}Yes ($CountryCode)${Font_Suffix}\n"
        return
    else
...
function MediaUnlockTest_StarPlus() {
    ...
    if [ -n "$region" ] && [ -z "$isUnavailable" ] && [[ "$inSupportedLocation" == "false" ]]; then
        ...
        echo -n -e "\r Star+:\t\t\t\t\t${Font_Green}Yes ($region)${Font_Suffix}\n"
        return
    elif [ -z "$region" ]; then
        echo -n -e "\r Star+:\t\t\t\t\t${Font_Red}No${Font_Suffix}\n"
        return
...
function MediaUnlockTest_DirecTVGO() {
    ...
    if [ -n "$isForbidden" ]; then
        ...
        echo -n -e "\r DirecTV Go:\t\t\t\t${Font_Green}Yes ($region)${Font_Suffix}\n"
        return
    fi
          ...
          echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
           echo -e " ${Font_SkyBlue}** Your network is 12: ${local_isp4}${Font_Suffix} "
            ...
            if [[ "$check4" != *"unreachable"* ]] && [[ "$check4" != *"Unreachable"* ]]; then
                ...
                echo -e "${Font_SkyBlue}???????IPv4,??...${Font_Suffix}"
                isv4=0
            fi
            echo ""
        fi
...
function CheckV6() {
    if [[ "$language" == "e" ]]; then
        ...
        if [[ "$NetworkType" == "4" ]]; then
            isv6=0
            echo -e "${Font_SkyBlue}?????detectIPv4??,??IPv6detect...${Font_Suffix}"
        else
            check6_1=$(curl $useNIC -fsL --write-out %{http_code} --output /dev/null --max-time 10 ipv6.google.com)
            check6_2=$(curl $useNIC -fsL --write-out %{http_code} --output /dev/null --max-time 10 ipv6.ip.sb)
            if [[ "$check6_1" -ne "000" ]] || [[ "$check6_2" -ne "000" ]]; then
                echo ""
                echo ""
                echo -e " ${Font_SkyBlue}** ????IPv6????${Font_Suffix} "
                echo "--------------------------------"
...
function Goodbye() {
    ...
    if [[ "$language" == "e" ]]; then
        ...
        echo -e ""
        echo -e "${Font_Yellow}detect????????:${TodayRunTimes}; ??????:${TotalRunTimes} ${Font_Suffix}"
        echo -e ""
        echo -e "${Font_SkyBlue}??????${Font_Suffix}"
        echo -e "================================================"
        echo -e "${Font_Yellow}DMIT TYO Lite?? ????cmi ????1Gbps???${Font_Suffix}"
        echo -e "${Font_SkyBlue}Lite?????(Tiny??): ${Font_Suffix}"
        echo -e "${Font_Green}?????Lite-Annually-Recur-30OFF? ${Font_Suffix}"
        echo -e "${Font_Green}??????Lite-Semi-Annually-Recur-20OFF? ${Font_Suffix}"
        echo -e "${Font_Red}???:${Font_Suffix}https://www.dmit.io/aff.php?aff=3198"
        echo -e "================================================"
...
function ScriptTitle() {
    if [[ "$language" == "e" ]]; then
        ...
        echo -e "????????????????"
        echo ""
        echo -e "${Font_Green}????${Font_Suffix} ${Font_Yellow}https://github.com/lmc999/RegionRestrictionCheck ${Font_Suffix}"
        echo -e "${Font_Green}BUG?????????TG??${Font_Suffix} ${Font_Yellow}https://t.me/gameaccelerate ${Font_Suffix}"
        echo -e "${Font_Purple}????OS: CentOS 6+, Ubuntu 14.04+, Debian 8+, MacOS, Android (Termux), iOS (iSH)${Font_Suffix}"
        echo ""
        echo -e " ** ????: $(date)"
        echo ""
    fi
